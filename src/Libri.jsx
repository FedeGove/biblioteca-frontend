...
function Libri({ ruolo }) {
  ...
  function handleAggiungi() {
    ...
    .then(data => {
      setLibri([...libri, data])
      setTitolo(''); setAutore(''); setAnno(''); setGenere('')
    })
    .catch(console.error)
  }
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-4">
            <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Totale libri</p>
            <p className="text-2xl font-bold text-white">{libri.length}</p>
          </div>
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-4">
            <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Disponibili</p>
            <p className="text-2xl font-bold text-green-400">{libri.filter(l => l.disponibile).length}</p>
          </div>
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-4">
            <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">In prestito</p>
            <p className="text-2xl font-bold text-amber-400">{libri.filter(l => !l.disponibile).length}</p>
          </div>
        </div>
            ...
            <div className="flex flex-col gap-2">
              {libri.length === 0 && (
                <div className="bg-slate-800 rounded-xl border border-slate-700 p-8 text-center">
                  <p className="text-slate-500 text-sm">Nessun libro nel catalogo</p>
                </div>
              )}
              {libri.map(libro => (
                <div key={libro.id} className="bg-slate-800 rounded-xl border border-slate-700 px-4 py-3 flex items-center justify-between hover:border-slate-600 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-white text-sm">{libro.titolo}</p>
                      <p className="text-xs text-slate-400">{libro.autore} · {libro.anno} · {libro.genere}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="bg-slate-800 rounded-xl border border-slate-700 p-4">
              <div className="flex flex-col gap-3">
                {[
                  { placeholder: 'Titolo', value: titolo, onChange: setTitolo },
                  { placeholder: 'Autore', value: autore, onChange: setAutore },
                  { placeholder: 'Anno', value: anno, onChange: setAnno },
                  { placeholder: 'Genere', value: genere, onChange: setGenere },
                ].map(field => (
                  <input
